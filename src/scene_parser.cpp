...
void SceneParser::parsePerspectiveCamera() {
    ...
    double aperture = readFloat();
    getToken(token);
    assert (!strcmp(token, "}"));
    camera = new PerspectiveCamera(center, direction.normalized(), up.normalized(), width, height, angle_radians, aperture);
}
...
Material *SceneParser::parseMaterial() {
    ...
    filename[0] = 0;
    Vector3f diffuseColor(1, 1, 1), specularColor(0, 0, 0), shineColor(0, 0, 0);
    double shininess = 0;
    ...
    while (true) {
        ...
        //     shininess = readFloat();
            ...
            getToken(filename);
            bmp = new Bmp();
            bmp->Input(string(filename));
            Vector2f stretch = readVector2f();
            bmp->SetStretch(stretch);
        } else if (strcmp(token, "shine") == 0) {
            shineColor = readVector3f();
            isShine = true;
